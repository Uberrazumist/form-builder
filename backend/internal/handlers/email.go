...
// cleanupRateLimitMap удаляет просроченные записи из rateLimitMap
func cleanupRateLimitMap() {
	rateLimitMux.Lock()
	defer rateLimitMux.Unlock()
	now := time.Now()
	for email, last := range rateLimitMap {
...
// periodicallyCleanupRateLimit запускает фоновую очистку каждые 5 минут
func init() {
	go func() {
